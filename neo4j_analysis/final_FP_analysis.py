...
#%%
top_fp = FP.sort_values("probability", ascending=False).head(100)
print(top_fp.head()) #compound_key_1 compound_key_2 y_true  probability  prediction 
top_fp.to_csv("final_top_false_positive_pairs.csv", index=False)
print("Saved: final_top_false_positive_pairs.csv")
# %%
#list compound pairs สำหรับ Neo4j
fp_pairs = list(zip(top_fp.compound_key_1, top_fp.compound_key_2))
print("FP pairs:", len(fp_pairs)) #FP pairs: 100
# %%
...
df.head(200)
# %%
df.to_csv("final_FP_evidence_table.csv", index=False)
print("Saved: final_FP_evidence_table.csv")
# %%
PD_REL = {
    "NEGATIVELY_MODULATES",
    "POSITIVELY_MODULATES",
...
final_table.to_csv("Summary_FP_evidence_table.csv", index=False)
print("Saved: Summary_FP_table.csv")
# %%
