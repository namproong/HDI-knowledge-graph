...
#%%
TP_compound = pd.read_csv("exTP_compound_level_evidence.csv")
print(len(TP_compound)) #2504
print(TP_compound.head()) 
#%%
print(TP_compound.columns) #Index(['species_tax_id', 'species_name', 'compound_key_1', 'compound_name_1', 'compound_key_2', 'compound_name_2', 'probability', 'prediction'],
#%%
TP_compound = TP_compound.reset_index(drop=True)
TP_compound["pair_id"] = TP_compound.index
#%%
Tp_pairs = list(zip(TP_compound.compound_key_1, TP_compound.compound_key_2))
print("Tp_pairs:", len(Tp_pairs)) #Tp_pairs: 2504
# %%
Tp_pairs = TP_compound[
    ["pair_id","compound_key_1","compound_key_2","probability"]
].drop_duplicates()
#%%
num_unique_pairs = TP_compound[
...
with driver.session(database="hdikgv3") as session:
    ...
    for _, row in Tp_pairs.iterrows():
        pair_id = row["pair_id"]
...
df_fp = df_fp.sort_values(
    by=["pair_id","species_name","probability"],
...
df_fp = df_fp[cols]
#%%
df_fp.to_csv("final_exTP_evidence_table.csv", index=False)
# %%
PD_REL = {
    "NEGATIVELY_MODULATES",
    "POSITIVELY_MODULATES",
...
final_table.head()
#%%
final_table.to_csv("Summary_exTP_evidence_table.csv", index=False)
