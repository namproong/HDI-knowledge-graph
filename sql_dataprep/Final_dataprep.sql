...
# insert จาก drugbank เข้า hdi_compound_metabolism 
INSERT INTO hdi.hdi_compound_metabolism (
    substrate_compound_key,
    metabolite_compound_key,
    metabolism_target_key,
...
# hdi_compound_target_assoc  ข้อมูล interaction จาก NPASS
CREATE TABLE hdi_stg_final_target_chembl_data (
    target_id    INT,
    target_name  TEXT,
    target_type  TEXT,
    chembl_id    TEXT
...
CREATE TABLE hdi.hdi_compound_target_assoc (
    compound_key   VARCHAR(40)
        REFERENCES hdi.hdi_compound(compound_key),
    target_key     BIGINT
        REFERENCES hdi.hdi_target(target_key),
    relation_type  TEXT DEFAULT 'INTERACTS_WITH',
    source         TEXT NOT NULL,   -- 'np'
    evidence_count INT,
    PRIMARY KEY (compound_key, target_key, source)
);
INSERT INTO hdi.hdi_compound_target_assoc (
    compound_key,
    target_key,
    source,
    evidence_count
...
CREATE TABLE hdi.hdi_action_type_mapping (
    action_type        TEXT PRIMARY KEY,
    parent_action_type TEXT NOT NULL
);
INSERT INTO hdi.hdi_action_type_mapping (action_type, parent_action_type) VALUES
-- POSITIVE MODULATOR
('ACTIVATOR', 'POSITIVE_MODULATOR'),
('AGONIST', 'POSITIVE_MODULATOR'),
('OPENER', 'POSITIVE_MODULATOR'),
('PARTIAL AGONIST', 'POSITIVE_MODULATOR'),
('POSITIVE ALLOSTERIC MODULATOR', 'POSITIVE_MODULATOR'),
('POSITIVE MODULATOR', 'POSITIVE_MODULATOR'),
('EXOGENOUS GENE', 'POSITIVE_MODULATOR'),
('EXOGENOUS PROTEIN', 'POSITIVE_MODULATOR'),

-- NEGATIVE MODULATOR
('ALLOSTERIC ANTAGONIST', 'NEGATIVE_MODULATOR'),
('ANTAGONIST', 'NEGATIVE_MODULATOR'),
('ANTISENSE INHIBITOR', 'NEGATIVE_MODULATOR'),
('BLOCKER', 'NEGATIVE_MODULATOR'),
('DEGRADER', 'NEGATIVE_MODULATOR'),
('GENE EDITING NEGATIVE MODULATOR', 'NEGATIVE_MODULATOR'),
('INHIBITOR', 'NEGATIVE_MODULATOR'),
('INVERSE AGONIST', 'NEGATIVE_MODULATOR'),
('NEGATIVE ALLOSTERIC MODULATOR', 'NEGATIVE_MODULATOR'),
('NEGATIVE MODULATOR', 'NEGATIVE_MODULATOR'),
('RELEASING AGENT', 'NEGATIVE_MODULATOR'),
('RNAI INHIBITOR', 'NEGATIVE_MODULATOR'),

-- ENZYMATIC ACTIVITY
('HYDROLYTIC ENZYME', 'ENZYMATIC_ACTIVITY'),
('OXIDATIVE ENZYME', 'ENZYMATIC_ACTIVITY'),
('PROTEOLYTIC ENZYME', 'ENZYMATIC_ACTIVITY'),
('REDUCING AGENT', 'ENZYMATIC_ACTIVITY'),
('METHYLATING AGENT', 'ENZYMATIC_ACTIVITY'),

-- BINDING ONLY
('BINDING AGENT', 'BINDING_ONLY'),
('SEQUESTERING AGENT', 'BINDING_ONLY'),
('CHELATING AGENT', 'BINDING_ONLY'),

-- STRUCTURAL MODULATION
('STABILISER', 'STRUCTURAL_MODULATION'),
('DISRUPTING AGENT', 'STRUCTURAL_MODULATION'),
('CROSS-LINKING AGENT', 'STRUCTURAL_MODULATION'),

-- SUBSTRATE
('SUBSTRATE', 'SUBSTRATE_TRANSPORT'),

-- IMMUNE MEDIATED
('VACCINE ANTIGEN', 'IMMUNE_MEDIATED'),

-- UNSPECIFIED
('MODULATOR', 'UNSPECIFIED_MODULATION'),
('OTHER', 'UNSPECIFIED_MODULATION');
ALTER TABLE hdi.hdi_compound_target_pd
ADD COLUMN parent_action_type TEXT;
UPDATE hdi.hdi_compound_target_pd pd
SET parent_action_type = atm.parent_action_type
FROM hdi.hdi_action_type_mapping atm
WHERE pd.action_type = atm.action_type;
